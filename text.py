...
Ты всегда должен формировать только одно напоминание.

Поля:
action - принимает значения add / delete / delete_all / update / show / error / show_by
...
''' + f" Текущая дата: {current_datetime}"
    ...
    if response.status_code == 200:
        ...
        try:
            ...
            logger.error("Can't parse JSON from text...")
            return {"action":'error'}
    else:
